...
\texttt{files} & Lưu metadata tài liệu: owner, cid, tên file, mime, kích thước, iv, access mode, version, vault, trạng thái hủy. \\ \hline
\texttt{tokens} & Lưu token truy cập, file tương ứng, người nhận, thời hạn, trạng thái thu hồi và mục đích token. \\ \hline
\texttt{encryption\_identities} & Lưu public key của ví và chữ ký xác nhận public key thuộc về ví đó. \\ \hline
\texttt{key\_envelopes} & Lưu key envelope cho token đích danh. \\ \hline
\texttt{vaults} & Lưu thông tin kho nhóm. \\ \hline
\texttt{vault\_members} & Lưu thành viên kho và vai trò owner/editor/viewer. \\ \hline
\texttt{vault\_threshold\_policies} & Lưu chính sách phê duyệt ngưỡng của kho. \\ \hline
\texttt{threshold\_file\_shares} & Lưu các share đã mã hóa cho từng thành viên. \\ \hline
\texttt{approval\_requests} & Lưu yêu cầu truy cập của người nhận. \\ \hline
\texttt{approval\_contributions} & Lưu đóng góp phê duyệt của từng người duyệt. \\ \hline
\texttt{audit\_events} & Lưu nhật ký hành động quan trọng; có trigger ngăn sửa/xóa. \\ \hline
\texttt{rate\_limits} & Lưu bộ đếm giới hạn request cho API nhạy cảm. \\ \hline
\end{longtable}
...
\hline
\textbf{Endpoint} & \textbf{Mục đích} \\ \hline
\texttt{POST /api/auth/start} & Sinh thông điệp nonce để ví ký. \\ \hline
\texttt{POST /api/auth/verify} & Xác minh chữ ký ví và tạo session. \\ \hline
\texttt{POST /api/storage/upload} & Nhận ciphertext và lưu vào provider storage. \\ \hline
\texttt{GET /api/storage/get} & Trả ciphertext nếu token hoặc approval hợp lệ. \\ \hline
\texttt{POST /api/files} & Tạo metadata file, token và share nếu có. \\ \hline
\texttt{DELETE /api/files/[id]} & Chủ sở hữu hủy tài liệu. \\ \hline
\texttt{POST /api/tokens/issue} & Cấp token mới cho file. \\ \hline
\texttt{POST /api/tokens/validate} & Kiểm tra token trước khi mở file. \\ \hline
\texttt{POST /api/tokens/revoke} & Thu hồi token. \\ \hline
\texttt{GET /api/approvals} & Liệt kê yêu cầu phê duyệt liên quan đến người dùng. \\ \hline
\texttt{POST /api/approvals/[id]/approve} & Gửi contribution phê duyệt cho yêu cầu. \\ \hline
\texttt{GET /api/audit} & Truy vấn audit event được phép xem. \\ \hline
\texttt{GET /api/health} & Kiểm tra database và provider storage. \\ \hline
\end{longtable}
...
\begin{enumerate}
  ...
  \item Client upload ciphertext lên \texttt{/api/storage/upload}.
  \item Storage trả về \texttt{cid}.
  \item Client gửi metadata, iv, access mode và key material hợp lệ tới \texttt{/api/files}.
...
Giao diện được chia thành các màn hình chính: trang giới thiệu, upload, dashboard và download/viewer. Thiết kế mới dùng nhận diện tối, có hiệu ứng chuyển động và logo riêng.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{02-upload-wizard.png}
\caption{Upload wizard và thiết lập quyền chia sẻ}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{03-dashboard-inbox.png}
\caption{Dashboard quản lý tài liệu, kho và yêu cầu}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{04-download-viewer.png}
