...
\textbf{Quy trình triển khai} được thực hiện theo các bước:
\begin{enumerate}
  \item Mã nguồn được đẩy lên nhánh \code{main} của GitHub.
  \item Railway nhận webhook, build Docker image và khởi động container mới.
  \item Container đọc biến môi trường, mở SQLite trong volume và kết nối tới R2.
  \item Railway gọi \code{/api/health}; phiên bản mới chỉ được xem là sẵn sàng khi database và storage provider đều hoạt động.
  \item Nhóm kiểm tra đăng nhập ví, upload ciphertext và mở dashboard trước khi dùng bản triển khai để demo.
\end{enumerate}

\textbf{Phân tách dữ liệu khi vận hành:}
\begin{itemize}
  \item Railway volume lưu metadata, token, yêu cầu phê duyệt và audit event.
  \item Cloudflare R2 chỉ lưu ciphertext, không lưu bản rõ của tài liệu.
  \item Trình duyệt giữ khóa giải mã hoặc khôi phục khóa theo chính sách quyền; server không chủ động giải mã nội dung.
\end{itemize}

Cách phân tách này giúp việc thay storage provider không làm thay đổi logic nghiệp vụ. Tuy nhiên, SQLite và R2 phải được sao lưu theo hai chính sách riêng. Nếu metadata bị mất thì ciphertext vẫn tồn tại nhưng hệ thống không còn thông tin để xác định chủ sở hữu, quyền truy cập và lịch sử audit.
